...
public class QuadraticProbingHashMap<K, V> {
    ...
    public QuadraticProbingHashMap(int initialCapacity) {
        table = (QuadraticProbingMapEntry<K, V>[])
                (new QuadraticProbingMapEntry[initialCapacity]);
        size = 0;
    }
    ...
    public V put(K key, V value) {
        ...
        // case for "probed table.length times" handled in
        // resizeBackingTable() method
        QuadraticProbingMapEntry<K, V> map =
                new QuadraticProbingMapEntry<>(key, value);
        if (table[putHelp(key)] != null && !table[putHelp(key)].isRemoved()) {
            V old = table[putHelp(key)].getValue();
            table[putHelp(key)].setValue(value);
            return old;
        } else {
            table[putHelp(key)] = map;
            size++;
            return null;
        }
    }
    ...
    /**
     * Helper method for put method above
     * enacts quadratic probing
     *
     * @param key k key
     * @return value
     */

    private int putHelp(K key) {
    ...
    public V remove(K key) {
        ...
        if (!containsKey(key)) {
            ...
            int i = Math.abs(key.hashCode() % table.length);
            int j;
            table[i].setRemoved(table[i].isRemoved());
            if (table[i].getKey().equals(key)) {
                ...
                for (j = 0; j < table.length; j++) {
                    j++;
                    i = Math.abs((key.hashCode() + (j ^ 2)) % table.length);
                }
                table[(key.hashCode() + (j ^ 2))
                        % table.length].setRemoved(true);
                return table[(key.hashCode() + (j ^ 2))
     ...
     */
    public Set<K> keySet() {
        Set<K> hash = new HashSet<>();
        for (int i = 0; i < table.length; i++) {
            if (table[i] != null && !table[i].isRemoved()) {
                hash.add(table[i].getKey());
            }
        }
        return hash;
    }
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("Length less than size");
        } else {
            QuadraticProbingMapEntry<K, V>[] old = table;
            QuadraticProbingMapEntry<K, V>[] curr =
                    (QuadraticProbingMapEntry<K, V>[]) new
                            QuadraticProbingMapEntry[length];
            for (int i = 0; i < old.length; i++) {
                if (old[i] != null && !old[i].isRemoved()) {
                    if (curr[old[i].getKey().hashCode()
                            % curr.length] != null) {
                        for (int j = 0; j < curr.length; j++) {
                            curr[(old[i].getKey().hashCode() + (j ^ 2))
                                    % curr.length] = old[i];
                            j++;
                            if (j >= old.length) {
                                resizeBackingTable(old.length * 2 + 1);
                            }
                        }
                    } else {
                        curr[old[i].getKey().hashCode() % curr.length] = old[i];
                    }
                }
            }
            table = curr;
    ...
    public void clear() {
        table = (QuadraticProbingMapEntry<K, V>[])
                (new QuadraticProbingMapEntry[INITIAL_CAPACITY]);
        size = 0;
    }
